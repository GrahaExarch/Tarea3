...
router.put('/:id', async (req, res) => {
  ...
  await db.run(sql, [location_name, company_id], err => {
      if (err) {
          res.status(500).json({ "error": err.message });
      }
      res.json({
...
router.delete('/', async (req, res) => {
  ...
  await db.run(sql, [location_name], err => {
      if (err) {
          res.status(500).json({ "error": err.message });
      }
      res.json({
